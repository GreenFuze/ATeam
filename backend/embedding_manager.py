...
class EmbeddingManager:
    ...
    def _load(self) -> None:
        with self._lock:
            ...
            if not os.path.exists(self.config_path):
                raise FileNotFoundError(f'Failed to find embedding configuration: {self.config_path}')
            
            with open(self.config_path, "r", encoding="utf-8") as f:
                data = yaml.safe_load(f) or {}
                self._selected_model = data["selected_model"]
                self._max_chunk_size = data["max_chunk_size"]
    ...
    def set_selected_embedding_model(self, model_id: str) -> None:
        assert isinstance(model_id, str), 'Given embedding model ID must be a string'
    ...
    def get_max_chunk_size(self) -> int:
        with self._lock:
            if not isinstance(self._max_chunk_size, int) or self._max_chunk_size <= 0:
                raise RuntimeError("Embedding max_chunk_size not set or illegal. Please configure it in settings or embedding.yaml.")
    ...
    def embed(self, texts: List[str]) -> List[List[float]]:
        ...
        if hasattr(model, "embed_batch") and callable(getattr(model, "embed_batch")):
            ...
            # Convert generator to list to ensure it's subscriptable
            if hasattr(embeddings, '__iter__') and not isinstance(embeddings, (list, tuple)):
